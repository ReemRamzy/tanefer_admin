    ...
    <v-container>
      ...
      <v-card class="pa-3 my-5">
          <v-row class="px-5" justify="space-between">
            ...
            <v-select
            :items="$store.state.hotelPaymentStatuses"
...
import { listHotelBookings, listClients, hotelPaymentStatuses, hotelStatuses, headers } from '../links'
...
export default {
  ...
  methods: {
    applyFilters () {
      ...
      if (this.bookingStatuses.length > 0) {
        for (let i = 0; i < this.bookingStatuses.length; i++) {
          term += `&hotel_booking_status_id[]=${this.bookingStatuses[i].id}`
        }
      }
      if (this.paymentStatuses.length > 0) {
        for (let i = 0; i < this.paymentStatuses.length; i++) {
          term += `&hotel_booking_status_id[]=${this.paymentStatuses[i].id}`
        }
      }
    ...
    getHotelBookings (term) {
      this.filterLoad = true
      this.loading = true
      this.$http.get(listHotelBookings(term), { headers: headers(this.$cookies.get('userToken')) }).then(response => {
        this.loading = false
        ...
        if (response.body.status === true) {
          ...
          this.hotelBookings = response.body.data.data
          this.loading = false
          this.filterLoad = false
        } else {
          this.currentPage = 0
    ...
    changePage (page) {
      ...
      term[1] += `&page=${page}`
      this.getHotelBookings(term[1])
    }
  },
  created () {
    this.getHotelBookings('page=1&per_page=15')
    if (this.$store.state.hotelPaymentStatuses.length === 0) {
      this.$http.get(hotelPaymentStatuses, { headers: headers(this.$cookies.get('userToken')) }).then(response => {
        if (response.body.status !== false) {
          this.$store.dispatch('setHotelPaymentStatuses', response.body.data)
        }
      })
    }
